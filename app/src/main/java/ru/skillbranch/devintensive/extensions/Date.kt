...
fun Date.humanizeDiff(): String {
    ...
    return when {
        ...
        hours > 26 -> "$prefix$days ${TimeUnits.DAY.plural(days)}$suffix"
        hours > 22 -> "${prefix}день$suffix"
        minutes > 75 -> "$prefix$hours ${TimeUnits.HOUR.plural(hours)}$suffix"
        minutes > 45 -> "час$suffix"
        seconds > 75 -> "$prefix$minutes ${TimeUnits.MINUTE.plural(minutes)}$suffix"
